 ...
 */
export const createLead = async (createLeadRequest: CreateLeadRequest, options?: Parameters<typeof customFetch>[1]): Promise<Lead> => {
...
export const getOptOutLeadUrl = (id: number,) => {




  return `/api/leads/${id}/opt-out`
}
 ...
 */
export const optOutLead = async (id: number, options?: Parameters<typeof customFetch>[1]): Promise<Lead> => {

  return customFetch<Lead>(getOptOutLeadUrl(id),
  {
    ...options,
    method: 'POST'
...
export const getOptOutLeadMutationOptions = <TError = ErrorType<ErrorResponse>,
    TContext = unknown>(options?: { mutation?:UseMutationOptions<Awaited<ReturnType<typeof optOutLead>>, TError,{id: number}, TContext>, request?: SecondParameter<typeof customFetch>}
): UseMutationOptions<Awaited<ReturnType<typeof optOutLead>>, TError,{id: number}, TContext> => {

const mutationKey = ['optOutLead'];
const {mutation: mutationOptions, request: requestOptions} = options ?
      options.mutation && 'mutationKey' in options.mutation && options.mutation.mutationKey ?
      options
      : {...options, mutation: {...options.mutation, mutationKey}}
      ...
      const mutationFn: MutationFunction<Awaited<ReturnType<typeof optOutLead>>, {id: number}> = (props) => {
          const {id} = props ?? {};

          return  optOutLead(id,requestOptions)
        }
  ...
  return  { mutationFn, ...mutationOptions }}
    ...
    export type OptOutLeadMutationResult = NonNullable<Awaited<ReturnType<typeof optOutLead>>>

    export type OptOutLeadMutationError = ErrorType<ErrorResponse>

    /**
 * @summary Opt a lead out
 */
export const useOptOutLead = <TError = ErrorType<ErrorResponse>,
    TContext = unknown>(options?: { mutation?:UseMutationOptions<Awaited<ReturnType<typeof optOutLead>>, TError,{id: number}, TContext>, request?: SecondParameter<typeof customFetch>}
 ): UseMutationResult<
        Awaited<ReturnType<typeof optOutLead>>,
        TError,
        {id: number},
        TContext
      > => {
      return useMutation(getOptOutLeadMutationOptions(options));
    }
